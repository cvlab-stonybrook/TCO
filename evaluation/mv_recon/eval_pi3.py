...
from ttt3r.models.ttt3r_pi3_tem3r_lora import TTT3R_Pi3_TEM3R_LoRA
from ttt3r.utils.interfaces import infer_mv_pointclouds_no_priors, infer_mv_pointclouds_w_priors_v2 as infer_mv_pointclouds_w_priors
from evaluation.mv_recon.utils import umeyama, accuracy, completion, umeyama_auto, robust_umeyama
from ttt3r.utils.messages import set_default_arg, write_csv
from ttt3r.utils.vis_utils import save_image_grid_auto
...
def main(hydra_cfg: DictConfig):
    ...
    pretrained_model_name_or_path: str = hydra_cfg.pi3.pretrained_model_name_or_path
    # 0. create model
    # model = TTT3R_Pi3_LoRA.from_pretrained(pretrained_model_name_or_path).to(hydra_cfg.device).eval()
    model = TTT3R_Pi3_TEM3R_LoRA.from_pretrained(pretrained_model_name_or_path).to(hydra_cfg.device).eval()
    logger = logging.getLogger("mv_recon-eval")
    logger.info(f"Loaded Pi3 from {pretrained_model_name_or_path}")

    for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
        from datetime import datetime
        wandb.init(project="ttt3r-mv-recon", name=f"{dataset_name}")
        ...
        for seq_idx, (seq_name, ids) in enumerate(seq_id_map.items(), start=1):
            if hydra_cfg.debug and dataset_name == "7scenes-sparse" and seq_idx not in [7, 8]:
                continue
            if hydra_cfg.debug and dataset_name == "DTU" and seq_idx not in [18, 20, 21]:
                continue
