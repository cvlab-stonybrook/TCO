...
from evo_utils import calculate_averages, load_traj, eval_metrics, plot_trajectory, get_tum_poses, save_tum_poses
...
def main(hydra_cfg: DictConfig):
    ...
    for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
        wandb.init(project="tco-vggt-relpose-dist", name=f"{dataset_name}")
        ...
        for seq_idx, seq in enumerate(seq_list):
            ...
            filelist = filelist[:hydra_cfg.pose_eval_num_frames] if hydra_cfg.pose_eval_num_frames is not None else filelist
            depth_filelist = depth_filelist[:hydra_cfg.pose_eval_num_frames] if hydra_cfg.pose_eval_num_frames is not None else depth_filelist

            # 4.2 real inference
            # pr_poses: c2w poses, (N, 3, 4), in torch
            # pr_intrs: focals + pps, (N, 3, 3), in numpy
            pr_poses, pr_intrs = infer_cameras_c2w_w_priors(filelist, depth_filelist, model, hydra_cfg, sample_idx=seq_idx, scene_name=seq)
            ...
            logger.info(f"Seq {seq_idx+1}/{len(seq_list)} {seq} | seq_len: {len(filelist)} | ATE: {ate:5.4f} | RPE-trans: {rpe_trans:5.4f} | RPE-rot: {rpe_rot:5.4f}")
